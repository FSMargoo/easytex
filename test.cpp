...
$\Alpha=\sum^{n-1}_{i=1}f(t_{i})(x_{i+1}-x_{i})=\lim{\N\to\inf}\int^{a}_{b}f_{\N}dx$，我们不妨将原式变形：$\int_{a}^{b}f(x)dx = \frac{-b + \sqrt {b^2-4ac}}{2a}=-\frac{b}{2a} + \frac{\sqrt {b^2-4ac}}{2a}$，令 $a=1, b=0, c=-1$，发现此时 $b^{2}-4ac=0+4=4$，此时 $\Alpha$ 在实数范围内存在 $\Alpha=\frac{4}{2}=2$，于是此时
问题转换为了寻找一个$[0, 1]$ 中的实值函数 $f$，使得 $\sum^{n-1}_{i=1}f(t_{i})(x_{i+1}-x_{i})=1$，这就非常简单了，
